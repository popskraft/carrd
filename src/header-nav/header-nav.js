...
(function() {
  ...
  function pickNav(header) {
    ...
    const fallbackNav = header.querySelector('.links-component');
    if (isValidNav(fallbackNav)) return fallbackNav;
  ...
  function ensureId(element, preferredBaseId, forbiddenId) {
    ...
    const baseId = preferredBaseId || 'header-nav-element';
    let candidateId = baseId;
    let suffix = 1;
    let existing = document.getElementById(candidateId);
    while (existing && existing !== element) {
      candidateId = baseId + '-' + suffix;
      suffix += 1;
      existing = document.getElementById(candidateId);
    }
